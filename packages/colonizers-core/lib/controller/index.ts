import HexCorner from "../game-objects/hex-corner";
...
export default class Controller extends BaseController {
  ...
  onRoad() {
    this.on("build-road", req => req.game.phase === "setup").then(
      this.isCurrentPlayer,
      this.initRoad,
      this.endTurn
    );

    this.on("build-road", req => req.game.phase === "playing").then(
      this.isCurrentPlayer,
  ...
  distributeResources(req, diceTotal, next) {
    ...
    tiles.forEach(function(tile: HexTile) {
      var _tiles = tile.getAdjacentCorners();
      _tiles
        .filter(function(corner: HexCorner) {
          return corner.owner != null;
        })
        .forEach(function(corner: HexCorner) {
