...
import HexEdge from "./game-objects/hex-edge";
...
export default class GameCoordinator {
  ...
  onBuildRoad(data, next) {
    const player = this.game.getPlayerById(data.playerId) as Player;
    const edge = this.game.board.edges.getById(data.buildId) as HexEdge;
